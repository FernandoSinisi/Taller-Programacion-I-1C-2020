...
int Cliente::recibir(void* buffer){
    ...
    while (socket_esta_conectado and (recv_bytes < msg_length)){
        ...
        if (s <= 0) {
            ...
            //TODO loguearError(LOG_ERROR_RECV);
    ...
    while (socket_esta_conectado and (recv_bytes < bytes_a_recibir)){
        ...
        /*
            if (s == -1) { 						//Hubo un error
                ...
                ::close(this->fd);
                throow ("Error en el receive" + std::string(strerror(errno)) + "\n");
...
void Cliente::setIsConnected(bool conectado) {
    esta_conectado = conectado;
}
