...
MenuJuegoVista::MenuJuegoVista(SDL_Renderer* renderizador) {
    ...
    this->texto_salir = new TextoSDL(this->renderizador, this->fuente_normal, COLOR_GRIS, TAM_FUENTE_NORMAL, MENSAJE_SALIR,
                                    ...
                                    ( ALTO_VENTANA - int ( TAM_FUENTE_NORMAL ) ) / 2 + (int) TAM_FUENTE_NORMAL * 4
                                    );
...
void MenuJuegoVista::setTextura(SDL_Texture *t) {
    this->textura = t;
}

void MenuJuegoVista::setRenderizador(SDL_Renderer *r) {
    this->renderizador = r;
}
...
SDL_Renderer *MenuJuegoVista::getRenderizador() {
    return this->renderizador;
}

void MenuJuegoVista::setMenuEspera() {
    SDL_RenderCopy(this->renderizador,this->cartel_espera,NULL,NULL);
    SDL_RenderPresent(this->renderizador);
}
