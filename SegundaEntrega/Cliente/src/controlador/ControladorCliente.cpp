...
bool ControladorCliente::iniciar() {
    ...
    this->nro_cliente = (Uint16) atoi(this->cola_mensajes->desencolarBloqueante().c_str());
...
void ControladorCliente::actualizar() {
    ...
    if (!this->juego_controlador->actualizar(&this->evento, this->cola_inputs) ||
    !this->continuar) {
        this->desconectarse();
        setFin(true);
        return;
        ...
        this->continuar = this->juego_controlador->iniciarNivel(&this->evento);
        this->cola_inputs->encolar(TECLA_LISTO);
        this->timeout = 0;
        this->cambiando_nivel = false;
        return;
        ...
        this->timeout = 0;
        this->cambiando_nivel = true;
        return;
    }

    else if (mensaje == MSJ_CERRAR_CLIENTE) {
        this->desconectarse();
        setFin(true);
        this->timeout = 0;
        return;
    ...
    if( ( (unsigned) this->timeout > TIMEOUT_RECEPCION_SV) && !(this->cambiando_nivel) ){
        this->desconectarse();
        return;
    }
...
std::string ControladorCliente::recibir() {
    if (this->cliente_modelo->estaActivo()) {
        ...
        this->cliente_modelo->cerrarConexion();
        return DESCONECTADO;
    }
