...
void Cliente::asignarNroCliente(size_t num_cliente) {
    this->hiloRecibir->asignarNroCliente((int)num_cliente);
}
...
void Cliente::setPingRecibido(bool estado){
    if (!estado){
        //Estado == false es cuando mando mensaje de ping al cliente
        this->t_actual_ping = SDL_GetTicks();
        if (this->t_actual_ping - this->t_inicial_ping > MS_PING_ALTO){
            this->ping_recibido = estado;
        }
    } else {
        //Estado == true es cuando mando recibo respuesta de ping del cliente
        this->ms_ping = SDL_GetTicks() - this->t_inicial_ping;
