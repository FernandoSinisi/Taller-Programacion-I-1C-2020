...
void ControladorServidor::inicializar() {
    ...
    if(this->controlador_juego->estanTodosLosJugadoresConVidasDesconectados()){
...
bool ControladorServidor::estaActivo(){
    unsigned long jugadores_desconectados = 0;
    ...
    if ( jugadores_desconectados == this->clientes.size()
         || this->controlador_juego->estanTodosLosJugadoresConVidasDesconectados() ){
...
int ControladorServidor::borrarClientesDesconectados(){
    ...
    for(long unsigned i=0; i < this->clientes.size();){
        ...
        if (this->clientes[i]->getFD() == FD_DEFECTO){       /// Si el jugador se desconecto
            //std::cout << "Borrando cliente";
            if (this->clientes[i]->estaListo()){     /// Si estaba listo
                diferencia += 1;
                //std::cout << " que estaba listo";
            }
...
void ControladorServidor::actualizar(){
    ...
    // Para evitar condición de desconexión en el cliente si todos los vivos están desconectados
    if(this->controlador_juego->estanTodosLosJugadoresConVidasDesconectados()){
...
void ControladorServidor::borrarClientesNoListos(){
    ...
    for (size_t i = 0; i < this->clientes.size(); ++i) {
        ...
        if (!(this->clientes[i]->estaListo() and this->clientes[i]->getFD() > FD_DEFECTO) and !esUsuarioValido(this->clientes[i]->getUsuario(), this->clientes[i]->getPassword())) {
...
void ControladorServidor::esperarTiempo(const unsigned int segs) {
...
void ControladorServidor::esperarTiempoConLock(const unsigned int segs) {
    this->mutex.lock();
    esperarTiempo(segs);
    this->mutex.unlock();
}
