...
/**
 * Clase que se encarga de ser el controlador del juego en un modelo MVC con Cliente - TP3_Servidor
 **/
...
public:
    ...
    /// Actualiza la tabla de puntajes del juego segun el estado de los personajes_vivos en el mismo
    void actualizarPuntajeJuego();

    /// Retorna un string decodificable que representa el estado de los personajes_vivos en el juego
    ...
    /// Retorna un string decodificable que contiene los puntajes de los personajes_vivos ordenados de forma descendente
    ...
    /// (Game over = destruccion de todos los personajes_vivos)
    /// TRUE -> partida perdida, FALSE -> partida no perdida
    bool juegoPerdido();

    /// Setea el personaje asociado al numero de cliente como desconectado en el juego
    ...
    bool estanTodosLosJugadoresConVidasDesconectados();
};
