...
Juego::Juego(int n_clientes) {
    this->personajes_con_vidas = new std::vector<Personaje*>();

    for(int nro_jugador = 0; nro_jugador < n_clientes; ++nro_jugador){
        this->personajes_con_vidas->push_back(new Personaje(X_PERSONAJE_DEFAULT,
    ...
    //this->nivel = new Nivel(this->personajes_con_vidas, this->tipos_enemigos_nivel_1, this->cantidad_enemigos_nivel_1, JEFE_DE_NIVEL[((this->numero_nivel + 1) % 3) + 1], this->numero_nivel);
    this->nivel = new Nivel(this->personajes_con_vidas, this->tipos_enemigos_nivel_1, this->cantidad_enemigos_nivel_1, this->numero_nivel);

    this->contador_para_reinicio = 0;
    this->jugadores_desconectados = 0;

    //Para arrancar desde el nivel 2 o 3 hacer this->avanzarNivel() 1 o 2 veces respectivamente
}

Juego::~Juego(){
    delete this->nivel;
    for (auto &personaje: *(this->personajes_con_vidas)){
        delete(personaje);
    }

    delete this->personajes_con_vidas;

}

void Juego::setNombreJugador(int n, std::string nombre) {
    (*personajes_con_vidas)[(unsigned long)n]->setNombre(nombre);
...
void Juego::actualizar(int input) {
    ...
    while(iterador < this->personajes_con_vidas->size()){
        if ((*(this->personajes_con_vidas))[iterador]->getNumeroPersonaje() == nro_personaje){
            personaje = iterador;
            break;
        }
        ++iterador;
    ...
    if(iterador == this->personajes_con_vidas->size()){
        return;
    }
        ...
        if (tecla == TECLA_LISTO) {
            (*(this->personajes_con_vidas))[personaje]->setListo(true);
            this->conectarPersonaje((*(this->personajes_con_vidas))[personaje]);
        }
        unsigned long jugadores_conectados = 0;
        unsigned long personajes_listos = 0;

        for (unsigned long i=0; i < this->personajes_con_vidas->size(); ++i){
            if ((*(this->personajes_con_vidas))[i]->estaListo()){
                personajes_listos += 1;
            }
            if ((*(this->personajes_con_vidas))[i]->estaConectado()){
        ...
        else if ((*(this->personajes_con_vidas))[personaje]->estaVivo()) {
...
//Se murieron todos los personajes_con_vidas y no les quedan vidas
bool Juego::gameOver() {
    return (this->personajes_con_vidas->size() == 0);
}
...
void Juego::reiniciarNivel(){
    delete(this->nivel);
    for (long unsigned personaje = 0; personaje < (this->personajes_con_vidas)->size();){
        if ((*(this->personajes_con_vidas))[personaje]->getVidas() == 0) {
            this->tabla_puntaje_vida[(size_t)(*(this->personajes_con_vidas))[personaje]->getNumeroPersonaje()].second[UI_CLAVE_PUNTAJE] = (*(this->personajes_con_vidas))[personaje]->getPuntaje();
            this->tabla_puntaje_vida[(size_t)(*(this->personajes_con_vidas))[personaje]->getNumeroPersonaje()].second[UI_CLAVE_PUNTAJE_NIVEL_1] = (*(this->personajes_con_vidas))[personaje]->getPuntajeNivel(1);
            this->tabla_puntaje_vida[(size_t)(*(this->personajes_con_vidas))[personaje]->getNumeroPersonaje()].second[UI_CLAVE_PUNTAJE_NIVEL_2] = (*(this->personajes_con_vidas))[personaje]->getPuntajeNivel(2);
            this->tabla_puntaje_vida[(size_t)(*(this->personajes_con_vidas))[personaje]->getNumeroPersonaje()].second[UI_CLAVE_PUNTAJE_NIVEL_3] = (*(this->personajes_con_vidas))[personaje]->getPuntajeNivel(3);
            this->tabla_puntaje_vida[(size_t)(*(this->personajes_con_vidas))[personaje]->getNumeroPersonaje()].second[UI_CLAVE_VIDAS] = 0;
            this->tabla_puntaje_vida[(size_t)(*(this->personajes_con_vidas))[personaje]->getNumeroPersonaje()].second[UI_CLAVE_ESTADO_PERSONAJE] = PERSONAJE_UI_MUERTO;
            this->tabla_puntaje_vida[(size_t)(*(this->personajes_con_vidas))[personaje]->getNumeroPersonaje()].second[UI_CLAVE_ENERGIA_PERSONAJE] = 0;
            this->tabla_puntaje_vida[(size_t)(*(this->personajes_con_vidas))[personaje]->getNumeroPersonaje()].second[UI_CLAVE_MODO_TEST] = (int)(*(this->personajes_con_vidas))[personaje]->getTest();
            delete((*(this->personajes_con_vidas))[personaje]);
            this->personajes_con_vidas->erase(this->personajes_con_vidas->begin() + (int)personaje);
        } else {
            (*(this->personajes_con_vidas))[personaje]->setPosX(X_PERSONAJE_DEFAULT);
            (*(this->personajes_con_vidas))[personaje]->setPosY(Y_PERSONAJE_DEFAULT + Y_PERSONAJE_DEFAULT * ((*(this->personajes_con_vidas))[personaje])->getNumeroPersonaje());
            (*(this->personajes_con_vidas))[personaje]->setListo(false);
            (*(this->personajes_con_vidas))[personaje]->reiniciarEnergia();
            (*(this->personajes_con_vidas))[personaje]->actualizar(POS_NINGUNA);
            (*(this->personajes_con_vidas))[personaje]->dejarDeDisparar();
            (*(this->personajes_con_vidas))[personaje]->dejarDePropulsionar();
            ++personaje;
        }
    }
    ...
    if (this->numero_nivel == 1) {
        ...
        this->nivel = new Nivel(this->personajes_con_vidas, this->tipos_enemigos_nivel_2, this->cantidad_enemigos_nivel_2, this->numero_nivel);
    }

    else{
        this->nivel = new Nivel(this->personajes_con_vidas, this->tipos_enemigos_nivel_3, this->cantidad_enemigos_nivel_3, this->numero_nivel);
    }
}
...
// personajes_con_vidas: SOLO los jugadores con vidas > 0
bool Juego::estanTodosLosJugadoresDesconectados() {
    return this->jugadores_desconectados == this->personajes_con_vidas->size();
}
