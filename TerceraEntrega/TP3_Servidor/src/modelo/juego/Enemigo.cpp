...
void Enemigo::actualizarDireccionContraria() {
    if (this->direccion_contraria){
        ...
        if(nueva_pos_x  < (ANCHO_FONDO - ANCHO_VENTANA)) {
...
void Enemigo::setTopePosicionX(int tope) {
    this->tope_posicion_x=tope;
}

int Enemigo::getTopePosicionX(){
    return this->tope_posicion_x;
}
